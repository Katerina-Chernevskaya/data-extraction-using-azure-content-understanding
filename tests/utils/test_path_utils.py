...
class TestBuildAdlsMarkdownFilePath(TestCase):
    ...
    def test_when_md_file_return_md_file(self):
        ...
        expected_path = "Collections/test_collection_id/test_lease_id/test_file.md"

        # act
        result = build_adls_markdown_file_path(doc_type, collection_id, file_name, lease_id)
    ...
    def test_when_doc_type_is_lease_and_lease_id_is_none(self):
        ...
        with self.assertRaises(ValueError) as context:
            build_adls_markdown_file_path(doc_type, collection_id, file_name, lease_id)
...
class TestBuildAdlsPdfFilePath(TestCase):
    ...
    def test_when_md_file_return_pdf_file(self):
        ...
        expected_path = "Collections/test_collection_id/test_lease_id/test_file.pdf"

        # act
        result = build_adls_pdf_file_path(doc_type, collection_id, file_name, lease_id)
    ...
    def test_when_lease_id_is_none_raises_value_error(self):
        ...
        # act & assert
        with self.assertRaises(ValueError) as context:
            build_adls_pdf_file_path(doc_type, collection_id, file_name, lease_id)
        self.assertEqual(str(context.exception), "Lease ID must be provided for COLLECTION document type.")
